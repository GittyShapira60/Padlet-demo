...
function resetFormState(
  ...
  setPollAnswers: (answers: string[]) => void,
) {
...
function getInitialState(postToEdit?: Post | null) {
  ...
  // If editing a poll, restore the existing answers from poll.options
  const pollAnswers =
...
export function useCreatePostModal({
  ...
  const addPollAnswer = useCallback(() => {
    setPollAnswers((prev) =>
      prev.length < MAX_POLL_ANSWERS ? [...prev, ''] : prev,
    );
  }, []);

  const removePollAnswer = useCallback((index: number) => {
    setPollAnswers((prev) =>
      prev.length > MIN_POLL_ANSWERS
  ...
  const hasEnoughPollAnswers =
    pollAnswers.filter((answer) => answer.trim().length > 0).length >=
    MIN_POLL_ANSWERS;
    ...
    const input = {
      ...
      imageFileName:
        activeTab === PostContentTabValues.Image
          ...
          : undefined,
      // Send poll answers only for poll type
      pollOptions:
        activeTab === PostContentTabValues.Poll
